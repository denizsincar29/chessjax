...
- **A** — анализ позиции движком Stockfish: оценка и лучший ход (см. раздел
  «Анализ»). Повторное нажатие (или **Esc**) — снять.
...
Также управлять можно кнопками-контролами на панели (⏮ ← → ▶ ⛶ Σ) и кнопками в
...
Клавиша **A** или кнопка **Σ** на панели — анализ текущей позиции движком
Stockfish (WASM, грузится с jsdelivr лениво — только при первом запросе, в
отдельном worker'е, не блокирует страницу). Через несколько секунд озвучивается
оценка и лучший ход: «Оценка: +0,4. Лучший ход: E2-E4», мат — «Мат в 3». Поле
лучшего хода подсвечивается синей рамкой. Повторное **A** или **Esc** — снять
подсветку и анализ.
